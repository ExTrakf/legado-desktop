...
/**
 * 桌面版 AndroidZipFile（原版基于 ParcelFileDescriptor，桌面用 RandomAccessFile 等价替代）。
 * 解压算法逐字保留，仅 I/O 原语由 PFD 换成 RandomAccessFile。
 * <p>
...
public class AndroidZipFile implements ZipConstants {
    ...
    private final int readLeShort(RandomAccessFile raf, byte[] b) throws IOException {
        raf.readFully(b, 0, 2);
        return (b[0] & 0xff) | (b[1] & 0xff) << 8;
    }
    ...
    private final int readLeInt(RandomAccessFile raf, byte[] b) throws IOException {
        raf.readFully(b, 0, 4);
        return ((b[0] & 0xff) | (b[1] & 0xff) << 8)
                | ((b[2] & 0xff) | (b[3] & 0xff) << 8) << 16;
    }
