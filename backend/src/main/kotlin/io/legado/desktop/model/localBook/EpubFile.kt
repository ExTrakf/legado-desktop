...
import io.legado.desktop.utils.encodeURI
    ...
    private fun readEpub(): EpubBook? {
        return kotlin.runCatching {
            ...
            BookHelp.getBookFile(book)?.let { file ->
                val openedZip = AndroidZipFile(file)
                zipFile = openedZip
        ...
        try {
            epubBook?.let {
                ...
                it.coverImage?.inputStream?.use { input ->
                    // 原 BitmapFactory.decodeStream + compress(JPEG)，桌面用 ImageIO 等价
                    BitmapUtils.writeJpeg(input.readBytes(), book.coverUrl!!)
