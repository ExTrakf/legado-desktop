...
/**
 * WebView JS 桥（对应原版 help/webView/WebJsExtensions.kt，桌面等价）。
 * funName request 分发逻辑逐字保留；activity（原版用其 lifecycleScope）裁剪，
 * JS 往返通道由 [DesktopWebView.evaluateJavascript] 提供（JCEF 实现）。
 */
@Suppress("unused")
class WebJsExtensions(
    ...
    private val webViewRef: WeakReference<DesktopWebView> = WeakReference(webView)
