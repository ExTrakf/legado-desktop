...
private fun File.readRuleDataKey(markerFileName: String): Result<String?> {
    val markerFile = File(DesktopEnv.homeDir.toFile(), markerFileName)
    return when {
...
private fun deleteRuleDataEntry(file: File, deleteRootDir: Boolean = false) {
    if (!FileUtils.delete(file, deleteRootDir)) {
        LogUtils.e(RULE_DATA_LOG_TAG, "Unable to delete invalid rule data: ${file.path}")
    }
}

object RuleBigDataHelp {

    private val ruleDataDir = FileUtils.createFolderIfNotExist(DesktopEnv.homeDir.toFile(), "ruleData")
