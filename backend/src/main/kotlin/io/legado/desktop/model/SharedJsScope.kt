...
import io.legado.desktop.help.SimpleLruCache
...
object SharedJsScope {
    ...
    private val cacheFolder by lazy { File(DesktopEnv.cacheDir.toFile(), "shareJs") }

    private val scopeMap = SimpleLruCache<String, WeakReference<ScriptBindings>>(16)
    ...
    private val cryptoScopeMap = SimpleLruCache<CryptoScopeKey, WeakReference<ScriptBindings>>(16)
    ...
    private fun loadCryptoJs(): String? {
        ...
        synchronized(cryptoTextLock) {
            cryptoJsText?.let { return it }
            return try {
                javaClass.getResourceAsStream("/scripts/cryptojs.min.js").bufferedReader().use { reader ->
    ...
    private fun evaluateJsLib(
        ...
        if (jsMap != null) {
            jsMap.values.forEach { value ->
                if (value.isAbsUrl()) {
                    val fileName = MD5Utils.md5Encode(value)
                    var js = SimpleACache.getAsString(fileName)
                    if (js == null) {
                        js = runBlocking {
