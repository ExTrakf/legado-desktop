...
import io.legado.desktop.constant.AppConst.dateFormat
...
import io.legado.desktop.help.http.StrResponse
import io.legado.desktop.help.source.SourceHelp
import io.legado.desktop.help.source.SourceVerificationHelp
import io.legado.desktop.help.source.VerificationResult
import io.legado.desktop.help.source.getSourceType
...
import io.legado.desktop.utils.EncodingDetect
import io.legado.desktop.utils.FileUtils
import io.legado.desktop.utils.GSON
import io.legado.desktop.utils.HtmlFormatter
import io.legado.desktop.utils.JsURL
import io.legado.desktop.utils.MD5Utils
import io.legado.desktop.utils.StringUtils
import io.legado.desktop.utils.UrlUtil
...
import io.legado.desktop.utils.isMainThread
import io.legado.desktop.utils.isSameOrDescendantOf
import io.legado.desktop.utils.mapAsync
import io.legado.desktop.utils.stackTraceStr
import io.legado.desktop.utils.toStringArray
...
interface JsExtensions : JsEncodeUtils {
    ...
    fun openVideoPlayer(url: String, title: String) {
        throw NoStackTraceException("桌面版不支持视频播放")
    }
...
//****************文件操作******************//
    ...
    fun getFile(path: String): File {
        val cachePath = DesktopEnv.cacheDir.toFile().absolutePath
        ...
        val file = File(aPath)
        val safeDir = DesktopEnv.cacheDir.toFile().parentFile!!
...
//******************文件操作************************//
    ...
    fun toast(msg: Any?) {
        rhinoContextOrNull?.ensureActive()
        LogUtils.e("桌面版","${getTag()}: ${msg.toString()}")
    }
    ...
    fun openUrl(url: String, mimeType: String? = null) {
        ...
        rhinoContextOrNull?.ensureActive()
        throw NoStackTraceException("桌面版不支持 openUrl 打开外部应用: $url")
    }
    ...
    fun showBrowser(
        ...
        // 桌面版不支持 WebView 弹窗（原 BottomWebViewDialog）
        throw NoStackTraceException("桌面版不支持 WebView 弹窗")
    }
