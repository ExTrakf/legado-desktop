...
import io.legado.desktop.data.entities.getFolderName
...
import io.legado.desktop.utils.createFileIfNotExist
...
object BookHelp {
    private val downloadDir: File = DesktopEnv.homeDir.toFile()
    ...
    suspend fun clearInvalidCache() {
        withContext(IO) {
            ...
            val filesDir = DesktopEnv.homeDir.toFile()
    ...
    fun hasImageContent(book: Book, bookChapter: BookChapter): Boolean {
        ...
        getContent(book, bookChapter)?.let {
            ...
            while (matcher.find()) {
                ...
                // 桌面版：暂不做图片解码有效性校验（原 BitmapFactory/SvgUtils）
            }
        }
        return ret
    }
    ...
    private fun checkImage(bytes: ByteArray): Boolean {
        // 桌面版：暂不做解码校验（原 BitmapFactory/SvgUtils 校验在 T6.2 图片处理时实现）
