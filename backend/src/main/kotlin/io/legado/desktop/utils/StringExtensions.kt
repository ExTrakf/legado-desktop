...
import io.legado.desktop.constant.AppPattern.dataUriRegex
import java.io.File
...
fun String.parseToUri(): String = this
...
fun String.cnCompare(other: String): Int {
    // 桌面版：JDK Collator（中文排序，语义与 Android ICU Collator 等价）
 ...
 * 将ip字符串转为InetAddress
 */
fun List<*>.parseIpsFromList(): List<InetAddress>? =
    mapNotNull { it?.toString()?.trim()?.takeIf { v -> v.isNotBlank() } }
        .mapNotNull { v ->
            runCatching { InetAddress.getByName(v) }.getOrNull()
        }
