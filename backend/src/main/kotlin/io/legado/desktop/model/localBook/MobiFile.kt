...
import io.legado.desktop.lib.mobi.KF6Book
import io.legado.desktop.lib.mobi.KF8Book
import io.legado.desktop.lib.mobi.MobiBook
import io.legado.desktop.lib.mobi.MobiReader
...
class MobiFile(var book: Book) : AutoCloseable {
    ...
    private fun readMobi(): MobiBook? {
        return kotlin.runCatching {
            close()
            BookHelp.getBookFile(book)?.let { file ->
                MobiReader().readMobi(file)
            }
        }.onFailure {
            close()
        ...
        try {
            mobiBook?.let {
                ...
                it.getCover()?.let { bytes ->
                    // 原 BitmapFactory.decodeByteArray + compress(JPEG)，桌面用 ImageIO 等价
                    BitmapUtils.writeJpeg(bytes, book.coverUrl!!)
                }
            }
        } catch (e: Exception) {
